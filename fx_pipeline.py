# fx_pipeline.py
# Single-file EURUSD pipeline with robust headless CSV ingest, per-window models,
# column-manifested training/prediction, and forward-holdout evaluation.
# Python 3.8+.
...
DEFAULT_CONFIG = {
    ...
    "paths": {
        ...
        "models_dir": "models",
        "feats_only": "data/features_only.parquet",
        "labs_only": "data/labels_only.parquet",
    },
    "windows": {
        "daily": {"start": "00:00", "end": "23:59"},
        "w2_10": {"start": "14:00", "end": "22:00"},  # 2am-10am ET ~ 11:00-19:00 PT? (custom)
    ...
    "exhaustion": {"ma_window": 20, "atr_window": 20, "atr_thresh": 1.5},
    "tick_volume": {
        "baseline_days": 20,
        "accel_fast": 5,
        "accel_slow": 20,
...
# Normalize header names (when present)
def _norm_col(s: str) -> str:
    return re.sub(r"\s+", " ", str(s).strip().lower())

def _read_csv_any_sep(path: Path, header="infer") -> pd.DataFrame:
    # Try automatic sniffing, then common delimiters
    try:
...
def _infer_server_offset_from_filename(path: Path) -> tuple[int, bool]:
    """Find 'GMT+2' and 'US-DST' hints in filenames."""
...
def _us_dst_active(ts: pd.Series) -> pd.Series:
    """Second Sunday of March 02:00 to First Sunday of Nov 02:00 (local)."""
...
_dt_re = re.compile(r"\d{4}[./-]\d{1,2}[./-]\d{1,2}(?:[ T]\d{1,2}:\d{2}:\d{2}(?:\.\d{1,6})?)?$")
...
def _detect_datetime_column(df: pd.DataFrame) -> Optional[pd.Series]:
    """Find datetime-like content in headered DataFrame (strings or epoch)."""
    ...
    direct = [
        ...
        "time gmt","time (gmt)","time (utc)","local time","bar time","bar start time","time"
    ]
    for k in direct:
        if k in cols_norm:
...
def read_tick_csv(path: Path) -> pd.DataFrame:
    """
    Return a DataFrame indexed by UTC timestamp with a single 'mid' column.
    Supports headered and headless CSVs. Headless forms:
      col0=full datetime, col1=price  OR  col0=date, col1=time, col2=bid, col3=ask
    """
    # First attempt as headered
    ...
    def _finalize(ts_text: pd.Series, mid: pd.Series) -> pd.DataFrame:
        # Parse datetimes (no infer_datetime_format)
    ...
    if "mid" in lower_map:
        ...
        for k in ["price", "close", "last", "c", "close price"]:
            if k in lower_map:
                ...
                break
        if mid is None:
            # Give headerless fallback one more shot
            ...
            if df2.shape[1] >= 3 and _series_is_numeric(df2["col1"]) and _series_is_numeric(df2["col2"]):
                ...
                raise ValueError(f"{path} missing usable price cols. Headers: {list(df.columns)}")
...
def tick_volume_features(
    ...
    # Pad to the same length per day
...
def pre_news_features(calendar_csv: str, window_bars: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    if not need.issubset(set(c.lower() for c in cal.columns.str.lower())):
        return pd.DataFrame(index=window_bars.index)
    cal.columns = [c.lower() for c in cal.columns]
    cal["event_time_utc"] = pd.to_datetime(cal["event_time_utc"], utc=True, errors="coerce")
    cal = cal[cal["impact"].isin([i.lower() for i in cfg["pre_news"]["impacts"]])]
...
def assemble_features(bars: pd.DataFrame, ticks_utc: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    for wkey in ["daily", "w2_10", "w3_12"]:
        ...
        feats["window"] = wkey
        out_blocks.append(feats)

    features = pd.concat(out_blocks, axis=0).sort_index()
    return features
...
def build_dataset(features: pd.DataFrame, labels: pd.DataFrame) -> pd.DataFrame:
    """Join per-window to avoid 'window' column collisions."""
    parts = []
    for wkey in ["daily", "w2_10", "w3_12"]:
        f = features[features["window"] == wkey].copy()
        if f.empty:
            continue
        l = labels[labels["window"] == wkey].copy()
        l = l.drop(columns=["window"], errors="ignore")
...
def build_models() -> Models:
    # Centered scaling (with_mean=True) to fix earlier issues
...
def _cols_for_window(df_cols: List[str], wkey: str) -> List[str]:
    cols = []
    cols += [c for c in df_cols if c.startswith(f"{wkey}_")]
    cols += [c for c in df_cols if c.startswith("dow_")]
    cols += [c for c in ("is_eom", "is_eoq", f"is_{wkey}")]
    seen = set()
...
def train_and_eval_per_window(df: pd.DataFrame, cfg: dict, models_dir: Path) -> Dict[str, Dict[str, float]]:
    ...
    for wkey in ["daily", "w2_10", "w3_12"]:
        ...
        y_cls = sub[f"{wkey}_up"].astype(int)
        y_reg = sub[f"{wkey}_pips"].astype(float)

        feat_cols = _cols_for_window(list(sub.columns), wkey)
        _save_cols(models_dir, wkey, feat_cols)

        X = sub[feat_cols].replace([np.inf, -np.inf], np.nan).fillna(0.0)
        ...
        for tr_idx, te_idx in splits:
            ...
            ytr_cls, yte_cls = y_cls.iloc[tr_idx], y_cls.iloc[te_idx]
            ytr_reg, yte_reg = y_reg.iloc[tr_idx], y_reg.iloc[te_idx]
        ...
        dump(best_cls, models_dir / f"model_cls_{wkey}.joblib")
        dump(best_reg, models_dir / f"model_reg_{wkey}.joblib")
...
def predict_next_day(features_or_dataset: pd.DataFrame, cfg: dict, models_dir: Path) -> Dict[str, Dict[str, float]]:
    """
    Accepts either the full dataset (features+labels+window) or features with 'window'.
    Drops label columns if present. Uses saved per-window column manifests.
    """
    df = features_or_dataset.copy()
    # Drop labels if present
    drop_cols = [c for c in df.columns if c.endswith("_pips") or c.endswith("_up")]
    if drop_cols:
    ...
    for wkey in ["daily", "w2_10", "w3_12"]:
        ...
        proba_up = float(cls.predict_proba(x_last)[:, 1][0])
        pips_pred = float(reg.predict(x_last)[0])
        key_out = wkey if wkey != "w2_10" else "2_10"
        out[key_out] = {
            "dir": "UP" if proba_up >= 0.5 else "DOWN",
...
def build_all(cfg: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    Path(cfg["paths"]["features_out"]).parent.mkdir(parents=True, exist_ok=True)
    dataset.to_parquet(cfg["paths"]["features_out"])
...
def evaluate_holdout(df: pd.DataFrame, cfg: dict, cutoff: pd.Timestamp) -> Dict[str, Dict[str, float]]:
    ...
    for wkey in ["daily", "w2_10", "w3_12"]:
        ...
        cls = load(models_dir / f"model_cls_{wkey}.joblib")
        reg = load(models_dir / f"model_reg_{wkey}.joblib")
        proba_up = cls.predict_proba(Xte)[:, 1]
        pred_up = (proba_up >= 0.5).astype(int)
        auc = roc_auc_score(yte_cls, proba_up) if len(np.unique(yte_cls)) > 1 else np.nan
        brier = brier_score_loss(yte_cls, proba_up)
        acc = accuracy_score(yte_cls, pred_up)
        mae = mean_absolute_error(yte_reg, reg.predict(Xte))
...
def main():
    ...
    sub.add_parser("train", help="Train per-window models and persist + column manifests")
    sub.add_parser("predict", help="Predict next session/day using latest dataset")
    ...
    if args.cmd == "build":
        ...
        df = pd.read_parquet(features_p)
        preds = predict_next_day(df, cfg, models_dir)
